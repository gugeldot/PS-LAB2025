# Number tycoon
This project was developed as part of an academic assignment for the Software patterns subject.

The project focuses on applying object-oriented design principles and multiple software design patterns in a real, functional application.

--- 

## Game Description
Number Tycoon is a real-time automation and strategy game where the player builds and manages a production network based on numerical flows.

The player must extract, transform, and consume numbers using mines, conveyors, mathematical modules, and wells in order to generate points and expand their infrastructure.  

---

## Installation and Execution
1. Clone the repository:
    `
    git clone https://github.com/gugeldot/PS-LAB2025.git
    `
2. To create the executable, navigate to the app/ folder:  
  On Windows:
    `
    bash dist/build_windows.sh
    `
  On linux: 
    `
    chmod +x dist/build_linux.sh
    ./dist/build_linux.sh
    `

3. Run the game in app/dist/your-os/GameApp
4. Enjoy

---

## Authors
This project was developed collaboratively by:

<p float="left">
  <a href="https://github.com/gugeldot" title="@gugeldot">
    <img src="https://github.com/gugeldot.png?size=50" width="50" style="vertical-align: middle; border-radius: 50%;">
    <span style="margin-left: 8px; vertical-align: middle;">@gugeldot</span>
  </a>
  &nbsp;&nbsp;&nbsp;
  <a href="https://github.com/DavidFraifer" title="@DavidFraifer">
    <img src="https://github.com/DavidFraifer.png?size=50" width="50" style="vertical-align: middle; border-radius: 50%;">
    <span style="margin-left: 8px; vertical-align: middle;">@DavidFraifer</span>
  </a>
  &nbsp;&nbsp;&nbsp;
  <a href="https://github.com/AlejandroTorres11" title="@AlejandroTorres11">
    <img src="https://github.com/AlejandroTorres11.png?size=50" width="50" style="vertical-align: middle; border-radius: 50%;">
    <span style="margin-left: 8px; vertical-align: middle;">@AlejandroTorres11</span>
  </a>
</p>

